...
class Vendedores extends CI_Controller{
    ...
    public function adicionar(){
        ...
        // precisam ser unicos
        $this->form_validation->set_rules('vendedor_rg', '', 'trim|required|max_length[45]|is_unique[vendedores.vendedor_rg]');
        ...
        //validation normal
        $this->form_validation->set_rules('vendedor_nome_completo', '', 'trim|required|min_length[2]|max_length[45]');
        ...
        $this->form_validation->set_rules('vendedor_endereco', '', 'trim|max_length[100]');
        ...
        $this->form_validation->set_rules('vendedor_obs', '', 'trim|max_length[200]');
    ...
    public function editar($vendedor_id = null){
        ...
        if(!$vendedor_id || !$this->core_model->get_by_id('vendedores', array('vendedor_id' => $vendedor_id))){
            ...
            $this->form_validation->set_rules('vendedor_email', '', 'trim|required|valid_email|max_length[100]|callback_check_email');
            $this->form_validation->set_rules('vendedor_celular', '', 'trim|max_length[15]|callback_check_celular');            
            $this->form_validation->set_rules('vendedor_telefone', '', 'trim|max_length[15]|callback_check_telefone');
            
            $this->form_validation->set_rules('vendedor_endereco', '', 'trim|max_length[100]');
            $this->form_validation->set_rules('vendedor_numero_endereco', '', 'trim|max_length[20]');
            $this->form_validation->set_rules('vendedor_bairro', '', 'trim|max_length[45]');
            // $this->form_validation->set_rules('vendedor_complemento', '', 'trim|max_length[100]');
            $this->form_validation->set_rules('vendedor_cidade', '', 'trim|required|max_length[50]');
            $this->form_validation->set_rules('vendedor_estado', '', 'trim|required|exact_length[2]');
            $this->form_validation->set_rules('vendedor_obs', '', 'trim|max_length[200]');
    ...
    public function deletar($vendedor_id = null){
        if(!$vendedor_id || !$this->core_model->get_by_id('vendedores', array('vendedor_id' => $vendedor_id))){
            $this->session->set_flashdata('error', 'vendedor não enontrado!');
            redirect('vendedores');
        }else{
            // deletar
    ...
    public function check_celular($vendedor_celular){
     ...
     public function check_rg($vendedor_rg){
        ...
        if($this->core_model->get_by_id('vendedores', array('vendedor_rg' => $vendedor_rg, 'vendedor_id !=' => $vendedor_id ))){
            $this->form_validation->set_message('check_rg', 'Este RG já exite');
            return false;
        }else{
            return true;
        }
